      ...
      function redirect($method,$url)
      {
         ...
         */
         function exchangeAccessToken()
         {
            ...
            $exchange_url.="&fb_exchange_token=".$this->getAccessToken();

            //get access token
            $content=file_get_contents($exchange_url);
         ...
         */
         function api($path,$method="GET",$params=array())
         {
            if($this->isLogin() == false)
            {
               throw new Exception("not login when call api method");
            }
         ...
         */
         function insightsValues($object_id,$metric,$since_date,$until_date)
         {
            ...
            */
            ...
            //----------- batch ----------------
            function batchApi($calls)
            {
               //$calls=func_get_args();

               $path='';

               //handle each api call's format
               //because batch need different format
               ...
               $batch= json_encode($calls);
               $params=array("batch"=>$batch);


               $ret=$this->api($path,"POST",$params);
